...
describe("buildPlanFactReport — полный план / прогноз (редизайн Сводного)", () => {
  const report = buildPlanFactReport(baseInput)
  const july = report.buckets.find((b) => b.key === "2026-07")!

  it("planRubFull = полный план месяца (10 дн × 320 000)", () => {
    expect(july.planRubFull).toBeCloseTo(3_200_000, 2)
  })

  it("planRubToDate = план за дни ≤ вчера (9 дн) = planRub текущего бакета", () => {
    expect(july.planRubToDate).toBeCloseTo(2_880_000, 2)
    expect(july.planRub).toBeCloseTo(july.planRubToDate, 2) // pro-rata для текущего
    expect(july.planRubFull).toBeGreaterThan(july.planRub) // полный > pro-rata
  })

  it("forecastRub = факт + план остатка = 3 640 000 + 320 000", () => {
    expect(july.forecastRub).toBeCloseTo(3_960_000, 2)
  })

  it("elapsedDays / totalDays = 9 / 31 (метка «9/31»)", () => {
    expect(july.elapsedDays).toBe(9)
    expect(july.totalDays).toBe(31)
  })

  it("kpi.planHorizonFullRub = сумма полных планов бакетов", () => {
    expect(report.kpi.planHorizonFullRub).toBeCloseTo(3_200_000, 2)
  })

  it("total.forecastRub = FAC = kpi.facPrimaryRub", () => {
    expect(report.total.forecastRub).toBeCloseTo(3_960_000, 2)
    expect(report.total.forecastRub).toBeCloseTo(report.kpi.facPrimaryRub, 2)
  })
})
