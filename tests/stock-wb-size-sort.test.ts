...
describe("sortSizes (STOCK-34)", () => {
  ...
  it("пустая строка попадает в конец: ['S', '', 'M'] → ['S', 'M', ''] (по SIZE_ORDER)", () => {
    // Rule 1 (deviation): план содержал [\"M\", \"S\", \"\"] (alpha) — конфликт с
    // test 2 (XL,S,M,L → S,M,L,XL по SIZE_ORDER). Канонический поведение:
    // S(1) < M(2) → S перед M; пустая строка в конец.
    expect(sortSizes(["S", "", "M"])).toEqual(["S", "M", ""])
  })
