...
interface SalesPlanFiltersProps {
...
export function SalesPlanFilters({
  directions,
  brands,
  categories,
  subcategories,
  ...
  basePath,
}: SalesPlanFiltersProps) {
  const router = useRouter()
  const searchParams = useSearchParams()

  // Каскадная фильтрация: дочерние опции сужаются под выбор родителя
  const visibleBrands =
    selectedDirectionIds.length === 0
      ? brands
      : brands.filter((b) => b.directionId && selectedDirectionIds.includes(b.directionId))
  ...
  function setDirections(values: string[]) {
    // При смене направления — тихо вычищаем невалидные бренды/категории/подкатегории
    const newBrandIds =
      values.length === 0
        ? selectedBrandIds
