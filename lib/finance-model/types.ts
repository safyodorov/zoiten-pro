...
export interface GlobalParams {
  ...
  /** Страховой запас товара, доля партии (напр. 0.15) — единоразовый буфер, держится, не продаётся */
  safetyStockPct: number
  /** Страховой запас ДС на счетах, доля месячной выручки (напр. 0.10) — минимальный остаток */
...
export interface ProfitMonthRow {
  ...
  cogs: number
  /** Операционные расходы = Выручка − Себест − Операционная прибыль */
  opex: number
  /** Операционная прибыль (до процентов) = U × Выручка */
  netProfit: number
  /** Проценты по кредиту за месяц (зависит от варианта финансирования) */
  interest: number
  /** Чистая прибыль после процентов = Операционная прибыль − Проценты */
  profitAfterInterest: number
  /** Реинвестировано (удержано) = max(0, profitAfterInterest) × reinvestRate */
  reinvested: number
  /** Выведено собственнику = max(0, profitAfterInterest) × (1 − reinvestRate) */
  withdrawn: number
}
...
export interface CreditAssessment {
  ...
  /** Дебиторка на конец горизонта, ₽ — деньги WB за продажи, приходящие после конца года */
  endingReceivable: number
  /** Кредиторка на конец горизонта, ₽ — платежи поставщикам по заказам, падающие после конца года */
  endingPayable: number
  /** Чистый долг с учётом ДЗ/КЗ, ₽ = остаток кредита − дебиторка + кредиторка */
  netDebtEnd: number
}
