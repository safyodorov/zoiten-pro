...
**Статус:** дизайн согласован (развилки закрыты), готов к планированию
...
**Фаза B — Второй фин-рез (стандартные условия):**
- `WbBoxTariff` + `fetchBoxTariffs` + sync-эндпоинт + cron + кнопка.
- Вывод `К_эфф(направление)` (срез по стоку) + AppSetting-ставки + `GlobalRatesBar`.
- `pricing-math` (calculatePricingStandard + опц. outputs, golden сохранён).
- 3 столбца + второй блок в модалке.
