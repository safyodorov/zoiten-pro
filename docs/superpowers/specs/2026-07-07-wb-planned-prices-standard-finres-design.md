...
**Фаза B v3 реализована** (quick 260708-lhb — обратная логистика невыкупа volume-based по
официальной формуле ВБ + ИРП-надбавка на Л_туда, статья «Возврат продавцу» убрана)
...
**v3 (2026-07-08) — обратная логистика volume-based + ИРП (`calculatePricingStandard` v3):**
- Обратная логистика невыкупа заменена с плоской ставки (`wbReturnLogisticsRub`, 50₽) на
  объёмную по официальной формуле ВБ: `reverseLogisticsForVolume(V, baseRub, perLiterRub)` —
  бэнды ≤1л фиксированы (23/26/29/30/32 ₽, верхняя граница включительно), V>1 =
  `baseRub + perLiterRub×(V−1)` (default 46₽ + 14₽/л, `AppSetting.wbReverseLogBaseRub`/
  `wbReverseLogPerLiterRub`). Выкуп по-прежнему входит через частоту в Л_эфф, не в саму ставку.
- Добавлена ИРП-надбавка (индекс распределения продаж) на Л_туда — цено-зависимая:
  `Л_туда = (delivBaseLiter + delivAddLiter × max(0,V−1)) × ИЛ + sellerPriceForIrp × (ИРП%/100)`,
  где `sellerPriceForIrp = priceBeforeDiscount × (1 − sellerDiscountPct/100)` (допущение: цена
  продавца до СПП). ИРП = `AppSetting.wbIrpPct` (ручное значение, 1.56%).
- Статья «Возврат продавцу» (`returnToSellerRub × defectRatePct/100`) УБРАНА из `profitStd` —
  дублировала расход обратной логистики и искажала прибыль-std.
- Ручные значения пользователя: **ИРП=1.56%, ИЛ=1.11** (было 1.0).
