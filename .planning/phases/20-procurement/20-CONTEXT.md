...
**Gathered:** 2026-05-20 (auto-defaults)
**Reviewed & updated:** 2026-06-09 (user reviewed via discuss-phase — 4 gray areas resolved, defaults confirmed)
**Status:** Ready for (re)planning — ⚠ existing 8+W0 plans (drafted 2026-05-20 from un-reviewed auto-defaults) are STALE and MUST be regenerated by /gsd:plan-phase

<domain>
## Phase Boundary

Phase 20 закрывает цикл procurement-операций ERP: учёт поставщиков с контактами и переговорами, оформление и tracking закупок с авто-расчётом депозитов/балансовых платежей по курсам ЦБ РФ. Раздел группируется в навигации как «Управление закупками» с тремя подразделами: Поставщики, Закупки, План закупок.
...
- **План закупок строится ЗАНОВО** в `/procurement/plan` (свежий раздел). Существующий MVP `/purchase-plan` (ProductIncoming) **НЕ трогаем** — переименовываем в нав/заголовке в «План закупок (временный)» и оставляем работать как есть, т.к. его данные подтягиваются в другие разделы. Данные из временного плана будут перенесены в новый позже (deferred). Детальный функционал нового `/procurement/plan` — лёгкий MVP/forecast, уточняется в research+planning.
- БД курсов валют ЦБ РФ + daily cron (forward-only, без historical backfill в v1)
...
- **D-01:** Новая модель `Supplier` с полями:
  ...
  - **Quick-select закупщика (UX, user 2026-06-09):** в селекторе Закупщика сотрудники, уже выбиравшиеся на эту роль (есть хотя бы у одного Supplier), показываются сверху для быстрого выбора; полный список сотрудников доступен поиском ниже. Реализуется через `distinct buyerEmployeeId` из существующих Supplier.
...
### Resolved by user (discuss-phase 2026-06-09)

1. **План закупок** — ✅ RESOLVED: существующий `/purchase-plan` MVP переименовать в «План закупок (временный)» и НЕ трогать (его данные подтягиваются в другие разделы); новый «План закупок» строить заново в `/procurement/plan`. Данные из временного переносим позже (deferred). Детальный функционал нового раздела — лёгкий MVP, уточняется в research/planning.
2. **Multi-supplier purchases** — ✅ RESOLVED: **один поставщик на закупку**. Если у товара несколько привязанных поставщиков — пользователь выбирает одного. (Подтверждает D-07 default.)
3. **Multi-currency** — ✅ RESOLVED: **одна валюта на закупку** (юань по умолчанию, можно сменить на всю закупку). Per-item currency НЕ нужен. (Подтверждает D-05 default.)
4. **Координаты для inspection address** — ✅ RESOLVED: **только Город + текстовый адрес + ссылка** (URL на Google/Yandex Maps). Отдельные lat/lng поля НЕ нужны. (Подтверждает D-03.)

### Defaults applied by Claude (low-stakes, not blocking — adjust during planning if needed)

5. **Связка с ProductCost** — при completion закупки НЕ создаём партию автоматически; ручной trigger / отдельная фаза. (deferred, см. Deferred Ideas #5)
6. **Sidebar порядок** — группа «Управление закупками» после «Себестоимость партий». «План закупок (временный)» остаётся видимым отдельным пунктом до миграции данных.
7. **PaymentStatus transitions** — статус PAID ставится вручную (кнопка/чек в UI закупки) + OVERDUE вычисляется live (`dueDate < now() AND paidDate IS NULL`). Без банк-интеграции.
8. **Notifications** для overdue — только UI badge в v1, без email/Telegram. (deferred)
9. **Hard/soft delete** — Supplier soft delete (D-20); Negotiation/SupplierContact сохраняются для истории.
10. **Уникальность Supplier.nameForeign** — без constraint (free text, возможны дубли имён).
11. **CBR backfill** — forward-only в v1 (cron заполняет с момента запуска), без historical backfill.
