...
**Все значения строго кратны 4** (4, 8, 12, 16, 24, 32). Никаких `py-1.5`, `p-1.5`, `gap-1.5` — 6px нарушает 8-point grid.

**Исключения, разрешённые для этой фазы:**
- Нативные `<select>` в модалке — высота `h-9` (36px, base-ui стандарт), а не 44px touch-target: приложение desktop-only (mobile acceptable не primary). 36px = 9×4, кратно 4.
...
| Heading | 16px | `text-base` | 500 medium | 1.4 (`leading-snug`) | `DialogTitle` модалки («Расчёт юнит-экономики»), заголовки разделов inputs/outputs внутри модалки, названия Product в Сводке, подписи групп ставок в шапке. |
| Display | 24px | `text-2xl` | 500 medium | 1.2 (`leading-tight`) | `<h1>` страницы «Управление ценами» (в layout.tsx как в `/cards/layout.tsx`). Один экземпляр на страницу. |

**Веса (строго два):**
- `font-normal` (400) — весь body, micro, labels, ячейки таблицы
- `font-medium` (500) — Heading, Display, значения «Прибыль/Re/ROI» в таблице, названия Product в Сводке, текущая цена в строке «Текущая цена», активные tab-ссылки

**Никаких `font-semibold` (600) или `font-bold` (700)** — только два веса. Визуальный вес достигается через размер (Display 24px сам по себе достаточно контрастен даже на medium).
...
- **Высота строки**: `h-10` (40px = 10×4, кратно 4) для ценовых строк, `h-14` (56px = 14×4) для единственной Сводки (увеличена из-за 3 подстрок: Наименование / Остаток / Скорость).
...
**Типографика и плотность 30-колонок:** все 30 расчётных ячеек используют **обязательные** классы:
...
- `px-2` (8px) + `py-1` (4px) — кратно 4, 8-point grid сохранён
- `h-8` (32px) — максимальная плотность внутри строки `h-10` (40px row height — paddings строки добавляют вертикальный ритм)
- `text-xs tabular-nums text-right` — числа выравниваются по правому краю, моноширинные цифры для вертикального сравнения столбцов
- `leading-tight` (1.25) — компенсирует плотный `py-1`, текст не обрезается по вертикали

Заголовки (`<TableHead>`) — `text-xs font-medium text-muted-foreground text-right px-2 py-2` (паддинг шапки чуть больше для визуального отделения) + свойство `writing-mode` НЕ применяется (не вертикальный текст — читаемо горизонтально, колонки узкие но не <60px).
