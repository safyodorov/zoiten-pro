...
| Phase quick-260710-jgs P01 | 18min | 3 tasks | 17 files |
...
| Phase quick-260714-ij9 P01 | 11min | 3 tasks | 9 files |
| Phase quick-260714-kke P01 | 4min | 2 tasks | 3 files |
...
- [Phase quick-260710-jgs]: Пулы storage/acceptance /finance/weekly: universeByNmId из ВСЕХ привязанных артикулов (не candidates), непривязанные nmId → account-level распределение
- [Phase quick-260710-jgs]: sales-reports: отдельный cooldown bucket finance-reports (1 req/мин), ровно 1 retry на 429 по Retry-After; std-сценарий Оферты остаётся моделью навсегда
...
| 260710-e7h | Движок недельного фин-отчёта lib/finance-weekly/ (pure computeWeeklyFinReport, дуал ИУ/Оферта, 2 мира затрат appliances/clothing, revenue-share пулы, кредит только appliances) + golden (nmId 165967746: ИУ +523.6/Оферта −2176.7, poolPerUnit≈175, clothing-credit-guard) — W-engine недельного фин-отчёта; tsc чисто, finance-weekly+pricing-math 63/63, изоляция подтверждена (нет pricing-math/prisma/next/react), деплоя нет (pure lib) | 2026-07-10 | a9e85a3, 8ad124a | Verified | [260710-e7h-lib-finance-weekly-golden](./quick/260710-e7h-lib-finance-weekly-golden/) |
| 260710-evz | W2a страница /finance/weekly: каркас + роллап-таблица на живых данных (data.ts loadWeeklyFinReportInputs: заказы WbCardFunnelDaily / реклама WbAdvertStatDaily / комиссии WbCard ИУ+оф / закупка ProductCost / кредит loadSummarySchedule appliances-only / модель N_std calculatePricingStandard; 2 мира затрат — delivery общий, credit только appliances, overhead/acceptance/storage per-universe; пулы manual в AppSetting) + saveWeeklyPools + WeeklyFinReportTable (sticky, Universe→Бренд→Артикул, ИУ/Оферта, водопад) + таб/section-title. Без модалки/план-факта/схемы. tsc чисто, finance-weekly+pricing-math 63/63. Стабы: пулы=0 (W3 банк), N_std TODO(W1) | 2026-07-10 | 0d26c19, 9cf3d10, ee4fd4c | Verified | [260710-evz-w2a-finance-weekly](./quick/260710-evz-w2a-finance-weekly/) |
| 260710-fr1 | W2b drill-down модалка артикула в /finance/weekly: движок аддитивно отдаёт per-unit CostBreakdown (16 статей) + qtyOrders на ScenarioResult/ArticleResult (netOfCommissionPerUnit=существующий cutPricePerUnit, новое только commissionPct — golden 523.6/−2176.7 не тронут); WeeklyFinArticleDialog (read-only shadcn Dialog, ИУ/Оферта колонки, статьи как в Excel «Показатели» + Прибыль/Выручка/Re/ROI); клик по строке артикула → модалка (useState ВЫШЕ empty-return, подытоги/грандтотал не кликабельны). tsc чисто, engine 20 (+5 breakdown) + pricing-math 48 = 68/68 | 2026-07-10 | 2ffc38a, ccc0926, 7697c1d | Verified | [260710-fr1-w2b-drill-down-finance-weekly-breakdown-](./quick/260710-fr1-w2b-drill-down-finance-weekly-breakdown-/) |
| 260710-gem | W2c план-факт в /finance/weekly: lib/finance-weekly/plan-fact.ts (loadWeeklyPlanFact: активная версия salesPlan.activeVersionId, план недели/месяца Σ planOrdersRub из SalesPlanVersionDay, факт МТД из WbCardFunnelDaily; pure distributePlanAcrossNmIds — 1 nmId весь план / пропорция по факту / поровну при нуле, сумма точная) + 4 unit-теста + productId в meta; таблица — колонки «План (нед), ₽»/«% вып. (нед)» + KPI-блок неделя/месяц-МТД (рендерится и при пустой неделе); wiring page.tsx. Движок/схема не тронуты (git-diff гейт). tsc чисто, 72/72 | 2026-07-10 | fd1f1f7, 7a82be9, f92ccf2 | Verified | [260710-gem-w2c-finance-weekly-salesplanversionday](./quick/260710-gem-w2c-finance-weekly-salesplanversionday/) |
| 260710-hkj | W2d фиксы /finance/weekly по сверке: (Ф1) базис одежды = gross выкупы WbSalesDaily + план clothing planBuyoutsRub + МТД из buyoutsRub; (Ф2) WbCommissionSnapshot + миграция 20260710 с backfill validFrom 2026-06-01 (старые ставки до роста оферты 07.07) + lib/wb-commission-history.ts (snapshotCommissionChanges хуки в wb-sync/wb-commission-iu; loadCommissionsForDate(weekEnd) fallback WbCard); (Ф3) реклама = Σ updSum × fullstats-доли (pure attributeSpendByShares); (Ф4) кредит = pure weeklyAccruedInterest остаток×ставка×7/365 ЗОЙТЕН (issueDate guard); (Ф5) таблица — полная иерархия Направление→Бренд→Категория→Подкатегория→Артикул (compareProductsByHierarchy) + подытоги per Направление + бейджи базиса. tsc чисто, 83/83, engine.ts не тронут | 2026-07-10 | 47f4e75, 99b0398, a0f2d0d, a6e4b20 | Verified | [260710-hkj-w2d-upd-accrual](./quick/260710-hkj-w2d-upd-accrual/) |
| 260710-jgs | W1 импорт отчёта реализации WB: WbRealizationWeekly (weekStart×nmId, nmId=0 account-level, 8 бакетов, reportIds) + миграция; lib/wb-realization-api.ts (finance/v1/sales-reports list+detailed, WB_FINANCE_TOKEN, cooldown 'finance-reports', деньги-строки, BigInt reportId, 429=1 повтор по Retry-After, pure classifyRealizationRow — отзывы по bonus_type_name, promotion отдельно); POST /api/wb-realization-sync (MANAGE, clean-replace) + кнопка «Реализация WB» + крон вт 05:50 (?week backfill) + лог-сверка Σ бакетов vs list-агрегаты; wiring: reviewWriteoff + N_iu=deliveryRub/qty + пулы storage/acceptance из реализации (universeByNmId из ВСЕХ привязанных, unresolved→account-level; manual fallback + бейдж источника). Std-Оферта осталась моделью (решение пользователя — на ИУ в отчётах нет std-логистики/хранения). tsc чисто, 110/110, engine.ts не тронут | 2026-07-10 | 6256b7c, 70b4173, 0fd0602 | Verified | [260710-jgs-w1-wb-sales-reports-n-iu](./quick/260710-jgs-w1-wb-sales-reports-n-iu/) |
| 260710-kvf | Фикс классификатора реализации WB по зонду реального API (первый синк: delivery/penalty/reviews=0): finance/v1 detailed — все поля camelCase, оператор = sellerOperName (supplier_oper_name НЕТ), деньги ПОЛЯМИ на строках. classifyRealizationRow → pure explodeRealizationRow (мульти-поле, вклады !== 0 — отрицательный forPay возвратов сохраняется): forPay/deliveryService/penalty/paidStorage/paidAcceptance + deduction суб-классификация по bonusTypeName («списание за отзыв»→reviewPoints, «продвижение»→promotion, иначе other) + rebillLogisticCost→other (не в расчёт); + resolvePoolTotals per-бакет (>0 → реализация, иначе manual; бейдж per пул) — paidStorage=0 на ИУ не затирает ручной пул. 132 гейтовых теста, engine.ts не тронут | 2026-07-10 | 383523d, d2f5434, 29a1dac, 6c65380 | | [260710-kvf-wb-api](./quick/260710-kvf-wb-api/) |
| 260710-lmb | W3a теги банка + авто-пулы фин-отчёта (гибрид): enum WeeklyCostTag {OPEX,CAPEX,DELIVERY_MP} + BankTransaction.weeklyCostTag + миграция 20260710_weekly_cost_tag + setWeeklyCostTag (BANK MANAGE) + select в /bank (только DEBIT); lib/finance-weekly/bank-pools.ts pure (sumBankPoolAutos Σ|amount| OPEX/DELIVERY_MP — CAPEX не запрашивается; resolveHybridPool: manual>0 → manual, 0=не задано → банк>0 → 0); wiring: гибрид deliveryToMp (shared) + appliances.overhead из банка; clothing.overhead = AppSetting financeWeekly.clothingOverheadFixedRub + недельная переменная (НЕ из банка); редактор пулов — подписи «банк: N ₽», бейджи вручную/из банка/—, поле фикс-одежды. 124 гейтовых теста, engine/realization не тронуты | 2026-07-10 | a42ee7a, 613d0b3, 83da519, b1b50ac | Verified | [260710-lmb-w3a](./quick/260710-lmb-w3a/) |
| 260710-mih | W3c фиксация недели /finance/weekly: WeeklyFinReportSnapshot (weekStart @unique, payloadJson Json — 1 строка/неделя, fixedBy FK SetNull) + миграция; lib/finance-weekly/snapshot.ts (pure payload v1, build/parse c version-guard→null, toIsoMonday) + live.ts (loadWeeklyLiveBundle — композиция из page, используется страницей и action'ом); fixWeeklyReport (MANAGE, СЕРВЕРНЫЙ пересбор, clean-replace upsert) + unfixWeeklyReport; page 3 режима: снапшот (рендер из payloadJson, редактор пулов скрыт, emerald-бейдж «Зафиксирована дата·кем») / stale-version (live-fallback + amber «Снапшот устарел») / live. Крона нет (фиксация — ручное решение). 134/134, engine.ts не тронут | 2026-07-10 | 1eba6e5, bf61f21 | Verified | [260710-mih-w3c-immutable](./quick/260710-mih-w3c-immutable/) |
...
| fast-260721 | Фикс отображения /finance/weekly: key={weekStart} на WeeklyFinReportControls — useState(manualPools) не реинициализировался при листании недель, поля пулов показывали state прежней недели (выглядело как «экономист не вводил»). Разбор 6 вопросов пользователя по отчёту недели 13–19.07: пулы в БД целы; хранение = 0 корректно (paid_storage API: начислено 136 240,55 ₽ и 100% сторнировано «скидка по спецусловиям»); отзывы 51 167 = точно raw detailed; реклама upd 145 340 vs кабинет 145 649 — лаг upd за 19.07; кредит 374 277 = 282 922 процентов (остаток тела × ставка × 7/365, 8 живых кредитов) + 91 355 JetLend комиссия+НДФЛ × 7/30; в водопаде нет статьи «Комиссия» (кандидат на доработку) | 2026-07-21 | 751ca89 |  | inline |
| 260721-o4b | 3 правки водопада затрат /finance/weekly: (1) бакет «Комиссия» (K−I)×H в обоих сценариях — аддитивно в CostWaterfall/ScenarioBreakdown/addToWaterfall; (2) хвосты рекламы/отзывов nmId вне candidates доводят waterfall.ad=updTotal / waterfall.review=Σ reviewPointsRub через новый опц. вход движка `waterfallTails` (лямп-сумма к iu+std после цикла articles, per-article не тронут); (3) хранение Оферты = calculatePricingStandard.storageAmount (тот же вызов, что logisticsEffAmount) вместо голого пула — решение пользователя, ИУ хранение по-прежнему 0. Golden nmId 165967746 пересчитан с реальными габаритами прод-БД (38×26×44 см → S=417.6 ₽/ед): Оферта profit −2176.7→−3847.1, ИУ 523.6 не тронут (3 shared-const describe-блока обновлены). tsc чисто, targeted vitest 160/160, полный npm run test 41 упавших/11 файлов = baseline (не регрессия) | 2026-07-21 | 87f78bf, 55668f6, 4ab6b97 | Verified | [260721-o4b-finance-weekly](./quick/260721-o4b-finance-weekly/) |
