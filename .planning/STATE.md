...
stopped_at: Completed quick 260710-mih — W3c фиксация недели /finance/weekly (immutable-снапшот, рендер из payloadJson)
last_updated: "2026-07-10T13:35:32.221Z"
progress:
  total_phases: 13
  completed_phases: 13
  total_plans: 51
...
| 260710-mja | Фикс парсера ИУ-комиссий `/api/wb-commission-iu`: WB сменил порядок колонок выгрузки (~07.07.2026), позиционный парсинг писал бы Самовывоз→fbw и Экспресс 3%→fbs/dbs. Новый pure `lib/wb-commission-iu-parser.ts`: детект формата по шапке («Маркетплейс»/«(FBW)» → новый, иначе легаси 1:1), ZWSP U+200B в шапке «Витрина (DBS)/Курьер» вычищается, дедуп subjectName (@unique), нераспознанная шапка → 400 с русской ошибкой; route тонкий (транзакция+snapshot W2d не тронуты); vitest 6/6. Параллельно данные на проде обновлены SQL: ИУ 7421 предметов из commission.xlsx, std из Tariffs API (Костюмы 34.5→43.5 FBW, все 22 категории), снапшоты истории validFrom=07.07 (253), старые ставки сохранены (снапшот 01.06 + архив WbCommissionIuBefore20260707) | 2026-07-10 | f197ec4 |  | [260710-mja-fix-excel-parser-api-wb-commission-iu-he](./quick/260710-mja-fix-excel-parser-api-wb-commission-iu-he/) |
...
Last session: 2026-07-10T13:35:32.214Z
Stopped at: Completed quick 260710-mih — W3c фиксация недели /finance/weekly (immutable-снапшот, рендер из payloadJson)
Resume file: None
