...
- [x] 28-03-PLAN.md — AssumptionsBar (MANAGE) + server actions (zod) + методология

## Milestone v1.3: Аналитика ниш

> ⚠ Планируется в изолированной ветке `gsd/phase-30-analytics` параллельно с милстоуном
> «Управление контентом» (другая сессия). Номер версии (v1.3) может конфликтовать — согласовать
> при слиянии. Артефакты фазы лежат в `.planning/phases/30-analytics/` и с контент-фазой не пересекаются.

**Goal:** Аналитик за минуту понимает, кто в топ-30 карточек ниши на WB и за счёт чего — на одном полноэкранном дашборде собраны карточка, воронка продаж (показы → CTR → корзина → заказ → выкуп), выручка и позиции в поиске (органика + реклама) по каждому из 30 SKU, с переключением вкладок, сортировкой и графиками динамики.

**Источник требований:** `.planning/phases/30-analytics/30-TZ-source.md` (ТЗ заказчика «Дашборд Топ-30 SKU в нише» v3, 2026-07-13). Falsifiable-требования фиксируются в `30-SPEC.md` (spec-phase).

**Стратегия:** Новый раздел `/analytics` поверх зрелого стека Next.js 15 + Prisma 6 + recharts. Три источника данных: (1) 6 JSON-файлов отчёта WB «Сравнение карточек» (загрузка пользователем, 6×5=30 SKU, определяют состав топ-30); (2) MPSTATS API (история позиций в поиске, токен `X-Mpstats-TOKEN`, 1 SKU = 1 запрос = 1 лимит тарифа — новая внешняя интеграция); (3) **переиспользование существующего скана карточек WB** (`lib/wb-api.ts` — curl `card.wb.ru/cards/v4/detail` + basket-CDN фото) для листинг-фото и характеристик конкурентных nmID. Новая `ERP_SECTION.ANALYTICS` + регистрация в `lib/sections.ts` / `lib/rbac.ts`.

### Phases (v1.3)

- [ ] **Phase 30: Аналитика — дашборд «Топ-30 SKU в нише»** — загрузка 6 detail-JSON + MPSTATS-интеграция + движок метрик/конверсий (from-sums) + скан карточек (реюз) + 5-вкладочный полноэкранный дашборд (Общая / Листинг / Характеристики / Статистика карточки / Статистика запросов) + сортировка по выручке / конверсии клик→заказ

### Phase Details (v1.3)

### Phase 30: Аналитика — дашборд «Топ-30 SKU в нише»

**Goal:** Пользователь загружает 6 файлов «Сравнение карточек» + указывает MPSTATS-токен → сервис собирает по 30 конкурентным SKU воронку (среднее за день + динамика), выручку, выкуп, медианную цену (−3% на WB-Кошелёк), листинг-фото и характеристики (скан карточек), позиции в поиске (органика/реклама из MPSTATS) → показывает полноэкранный дашборд с 5 вкладками, единой сортировкой (выручка / конверсия клик→заказ), графиками динамики «одна метрика = один график» и тепловой картой позиций «запросы × дни».

**Источник:** `.planning/phases/30-analytics/30-TZ-source.md`
**Requirements**: `ANL-*` — фиксируются в `30-SPEC.md`, затем в `REQUIREMENTS.md # Phase 30 Requirements` (ещё не разложены).
**Depends on:** переиспользует `lib/wb-api.ts` (curl `card.wb.ru/cards/v4/detail` — цена/СПП/рейтинг + basket-CDN фото), паттерн RBAC-раздела (`lib/sections.ts`, `lib/rbac.ts`, enum `ERP_SECTION`), `recharts` (графики динамики), sticky-table pattern. **Функционально независим от товаров компании** — анализ КОНКУРЕНТОВ по произвольным nmID, не завязан на `Product`/`MarketplaceArticle`.
**Секция:** новая `ERP_SECTION.ANALYTICS` (read-heavy; write = загрузка файлов / запуск MPSTATS-синка).

**Открытые вопросы (ТЗ §7 + выявленные — на discuss-phase):**
  1. Привязка окна дат MPSTATS (по умолч. 30 дн) к периоду «Сравнения карточек»; общая шкала дат для обеих частей.
  2. Точный механизм скана карточек КОНКУРЕНТОВ (фото листинга + характеристики для произвольных nmID) — подтвердить, что curl-механизм `lib/wb-api.ts` это покрывает; характеристики конкурентов ≠ Content API продавца (`/content/v2/get/cards/list` — только свои карточки).
  3. Источник списка «все запросы ниши с частотностью > 500» — из MPSTATS (какой эндпоинт) или из `searchQueries` файлов details?
  4. UX загрузки 6 файлов + персистентность: одна ниша vs много ниш, повторный прогон, хранение в БД vs эфемерно.
  5. Лимиты/деградация MPSTATS (30 запросов + пагинация) — прогресс, кэш, обработка 429/лимита тарифа.

**Success Criteria (предварительно из ТЗ — финализируется в `30-SPEC.md`):**
  1. Пользователь загружает 6 detail-JSON → состав топ-30 определяется ими (30 SKU), никакого доп. отбора; дубликаты/ошибочные файлы обрабатываются явно.
  2. Метрики среднего за месяц считаются «от сумм» (Σ переходов/Σ показов и т.д.), а показы/заказы/сумма — как месяц ÷ 30; медианная цена = средняя за период с поправкой −3%; конверсия клик→заказ = клик→корзина × корзина→заказ.
  3. 5 вкладок, во всех видны все 30 строк (1 строка = 1 SKU): «Общая информация» (по умолч.), «Листинг» (5 фото крупно), «Характеристики», «Статистика карточки» (столбец средних + графики динамики по выбранным метрикам), «Статистика запросов» (тепловая карта позиций «запросы × дни», средняя позиция по дням присутствия).
  4. Единая сортировка топ-30 по выручке или конверсии клик→заказ действует на всех вкладках.
