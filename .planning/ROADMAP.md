...
3. **Детальная карточка кредита** — график выплат (тело + проценты) + доп. информация по кредиту.
4. **Сводный график выплат** — горизонтальная таблица с настраиваемой разбивкой (день / неделя / месяц): крайний левый столбец = кредиты + их инфо, далее вправо — выплаты в ячейках, соответствующих датам.

**Источник данных:** `C:\Users\User\zoiten-pro\Кредиты.xlsx`
- *Лист1* — дневной график тела долга по кредитам (колонки = кредиты, сгруппированы по орг) + секции «Новые кредиты» и «Балансы» (остаток по орг + Итого).
- *Лист2* — метаданные кредитов (сумма / ставка % / срок мес / банк / орг / № КД) + помесячные суммы основной долг + проценты, с per-org и общими итогами.
...
- [x] 21-03-PLAN.md — lib/loan-math + server actions (credits + bank)
- [x] 21-04-PLAN.md — Seed-скрипт из Кредиты.xlsx + сверка с Лист2
- [x] 21-05-PLAN.md — Список кредитов + фильтры + LoanModal CRUD
