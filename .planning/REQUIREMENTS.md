...
- [x] **SP-07**: План/факт с отклонением (₽ и %) за день/неделю/месяц/квартал/полугодие/весь горизонт («Итог» = горизонт H2, календарно-годовой тотал за 2026 не показывать; year-бакет в движке — для будущих лет). Таб «Сводный» (`/sales-plan`): `PlanFactMatrix` + KPI-карточки (в т.ч. «отставание от ИУ нарастающим») + график (recharts, факт-bars/план-line/ИУ-dashed). `buildPlanFactReport()` — pure: pro-rata текущего бакета, FAC-прогноз, накопительный итог.
- [x] **SP-10**: Факт продаж из `WbCardFunnelDaily` на лету (без новых таблиц факта). Кабинет = токен `WB_API_TOKEN`. Два разреза: company-level (все nmId, сравнение с ИУ) и product-level (через MarketplaceArticle). Разница — строка «Вне плана» (непривязанные nmId). Settle-лаг 7 дней (`factSettled=false` для свежих дней).
...
## Phase 26 Requirements — План продаж: рабочая правка уровней

Added 2026-07-05. Расширение `/sales-plan` под ручную рабочую модель пользователя (по итогам UAT Phase 25). Секция остаётся `SALES` (write — `SALES MANAGE`).

- [x] **SP-15**: Автопротяжка месячного уровня вперёд + сброс ручных. При сохранении уровня (заказов/шт в день) в месяц — опция «распространить на последующие месяцы» (по умолчанию **вкл**): уровень записывается во все месяцы горизонта ≥ выбранного, у которых **нет собственного явного** `SalesPlanMonthLevel` (авто-месяцы). Месяцы с ручным уровнем протяжка **не перезаписывает**. Отжатая опция → пишется только выбранный месяц. Механизм избавления от ручных: поштучный сброс уровня → авто (крестик ✕ в `ProductPlanCell` — сделать заметнее) + **массовый** «Сбросить ручные → авто» (по товару / по месяцу / по выбранным ячейкам). Тест «протяжка не перезаписывает ручные».
- [x] **SP-16**: Явное предупреждение о срезе плана. Когда целевой уровень невыполним из-за нехватки стока / позднего прихода — движок уже режет `orders=min(ставка,сток)`; вывести **причину** в матрицу «Товары»: бейдж «срезано −X% · ближайший приход dd.mm» на ячейке месяца; полностью нулевой месяц из-за отсутствия товара → плашка «нет товара, ближайший приход dd.mm» (или «товар придёт в <месяц>»). Данные из движка (`firstStockoutDate`, `lostRubToStockout`, `lostUnitsToStockout`, флаг `isLate` виртуальной закупки). UI-only, расчёт не меняется.
- [x] **SP-17**: Динамический roll-forward виртуальных отгрузок. «Отгрузку не сделали реальной» = виртуальную закупку не сконвертировали в `Purchase`. Виртуальная закупка с `orderDate < today` сдвигается вперёд: `orderDate → today`, `expectedArrivalDate → today + leadTime` (инвариант «не прошлым числом»). Сейчас инвариант держится только для авто-`SUGGESTED` при регенерации — **распространить на `ACCEPTED`** (сейчас ACCEPTED в симуляции стоит на исходной дате прихода и не двигается). **Ежедневный крон-пересчёт** (dispatcher, ~04:40 МСК) регенерирует авто-SUGGESTED + сдвигает просроченные ACCEPTED, чтобы план сам отражал «нет товара — нет продаж» без ручного клика. Тест инварианта «не прошлым числом» для ACCEPTED.

## Phase 27 Requirements — План продаж: ABC-статус + флаг «заказываем»

Added 2026-07-05. Расширение `/sales-plan «Товары»`. Секция `SALES` (write — `SALES MANAGE`).

- [ ] **SP-18**: В `/sales-plan «Товары»` — ABC-статус товара (A/B/C) бейджем с **инлайн-сменой** прямо в строке; смена пишет `Product.abcStatus` (enum уже существует) **глобально** (видно во всех разделах, где показывается статус). Server action `updateProductAbcStatus(productId, 'A'|'B'|'C'|null)` (SALES MANAGE, revalidate). Пустой статус допустим.
- [ ] **SP-19**: Флаг **«заказываем / не заказываем»** — новое глобальное поле `Product.orderEnabled Boolean @default(true)` (рукописная миграция + `prisma migrate deploy`). Эффективное значение = `abcStatus !== 'C' && orderEnabled`: статус **C = вывод из ассортимента** (тумблер off + заблокирован); **A/B** — тумблер по выбору. Гейт движка: товары с `effectiveOrderEnabled=false` **исключаются из генерации виртуальных закупок** (`suggestVirtualPurchases` / `regenerateVirtualPurchasesInternal` skip). План продаж будущих периодов — **распродажа остатка, потом 0** (движок `orders=min(ставка,сток)` не меняем; без пополнений сток истощается сам). Тумблер в строке товара `/sales-plan «Товары»`; server action `updateProductOrderEnabled(productId, bool)` (SALES MANAGE) + регенерация VP + revalidate. Тесты: C/не-заказываем → 0 виртуальных закупок; A/B заказываем → без изменений.
