...
Данные комиссий на проде обновлены SQL-ом (сессия 2026-07-10, две итерации — commission.xlsx оказался **офертой**, не ИУ, уточнение пользователя). Итог:
- `WbCard.commFbwStd/commFbsStd` — новая оферта из Tariffs API (Костюмы 34.5→43.5 FBW, все 22 категории);
- ИУ (`WbCommissionIu` + `WbCard.commFbwIu/commFbsIu`) — оставлена ПРЕЖНЕЙ (Костюмы 39.472), до отдельной ИУ-выгрузки от WB;
- снапшоты истории: 2026-06-01 (iu 39.472 / std 34.5) и 2026-07-07 (iu без изменений / std новая), 253 карточки;
- справочные таблицы вне Prisma-схемы: `WbCommissionIuBefore20260707` (копия старой ИУ, 7247) и `WbCommissionOffer20260707` (новая оферта-выгрузка, 7421).

⚠ Урок: выгрузка «комиссии» из кабинета WB (шапка с «Маркетплейс (FBS)»/«Склад WB (FBW)») — это таблица ОФЕРТЫ (совпадает с Tariffs API), ИУ приходит отдельным файлом.
